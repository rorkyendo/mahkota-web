    ...
    <ul class="nav">
      <li class="nav-profile">
        ...
        <div class="info">
          ...
          <small style="font-size:8px;"><?php echo $this->session->userdata('nama_toko');?></small>
        </div>
      </li>
    </ul>
    <!-- end sidebar user -->
    <!-- begin sidebar nav -->
    <ul class="nav <?php if($this->session->userdata('buka_kasir') == TRUE){ echo 'hidden';}?>">
...
<!-- end #sidebar -->
<?php $getTiket = $this->GeneralModel->get_by_id_general('ms_tiket','status_tiket','open');?>
<?php $getTiketProses = $this->GeneralModel->get_by_id_general('ms_tiket','status_tiket','process');?>
<?php $cekMembership = $this->GeneralModel->get_by_multi_id_general('ms_transaksi','jenis_transaksi','member','payment_status','process');?>
<script>
  if (<?php echo COUNT($getTiket);?> > 99) {
    $('<span class="badge pull-right bg-red">99+</span>').insertBefore('.fa-inbox')
  }else{
    $('<span class="badge pull-right bg-red"><?php echo COUNT($getTiket);?></span>').insertBefore('.fa-inbox')
  }
  if (<?php echo COUNT($getTiketProses);?> > 99) {
    $('<span class="badge pull-right bg-blue">99+</span>').insertBefore('.fa-inbox')
  }else{
    $('<span class="badge pull-right bg-blue"><?php echo COUNT($getTiketProses);?></span>').insertBefore('.fa-inbox')
  }
  if (<?php echo COUNT($cekMembership);?> > 99) {
    $('<span class="badge pull-right bg-red">99+</span>').insertBefore('.fa-id-card')
  }else{
    $('<span class="badge pull-right bg-red"><?php echo COUNT($cekMembership);?></span>').insertBefore('.fa-id-card')
  }
</script>
